...
with hdf.File('./targetedRealistic_masses.hdf5', 'r') as f:
    ...
    mill = dset['M200c', 'ML_pred_3d']

with hdf.File('./targetedRealistic_masses_Buzzard.hdf5', 'r') as f:
    dset = f[list(f.keys())[0]]
    buzz = dset['M200c', 'ML_pred_3d']

with hdf.File('../analysis/biasandScatter_target.hdf5', 'r') as f:
    dset = f[list(f.keys())[0]]
    bias = dset['centers', 'ML_bias_3d']
...
for d, c, style, zo in zip([mill, buzz], ['#e24a33', '#467821'], ['-', '--'],
    ...
    if zo == 2:
        print('3d for biases')
        bins = pyl.arange(11.5, 16, 0.1)
        indx = pyl.digitize(d['ML_pred_3d'], bins)
        newd = d['ML_pred_3d'] - bias['ML_bias_3d'][indx]

        err = calc_err(10**newd, d['M200c'])
        bins = bayesian_blocks(pyl.log10(d['M200c']), p0=0.01)

        y_ = astStats.runningStatistic(pyl.log10(d['M200c']), newd,
        pyl.percentile, binNumber=bins, q=[16, 50, 84])
        quants = pyl.array(y_[1])
        ax.plot(y_[0], quants[:, 1], style, c='red', zorder=zo)

        y_ = astStats.runningStatistic(pyl.log10(d['M200c']), err,
                pyl.percentile, binNumber=bins, q=[16, 50, 84])
        quants = pyl.array(y_[1])
        axs.plot(y_[0], quants[:, 1], style, c='red', zorder=zo)
#		axs.fill_between(y_[0], quants[:, 2], quants[:, 0], facecolor=,
#			alpha=0.4, edgecolor=c)
