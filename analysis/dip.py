...
def plot_dip(histogram=None, idxs=None):
    ...
    plt.plot(idxs[len(left)-1:len(left)+len(left_part) - 1], left_part,
            color='gray')
    plt.plot(idxs[-len(right):], right, color='blue')
    plt.plot(idxs[len(cdf) - len(right) + 1 - len(right_part):len(cdf) -
        len(right) + 1], right_part, color='gray')
    ...
    plt.vlines(x=idxs[:len(left)][l_dip_idxs],
            ymin=cdf[:len(left)][l_dip_idxs], ymax =\
            cdf[:len(left)][l_dip_idxs] - the_dip)
    plt.vlines(x=idxs[-len(right):][r_dip_idxs],
            ymin=cdf[-len(right)-1:-1][r_dip_idxs], ymax =\
            cdf[-len(right)-1:][r_dip_idxs] + the_dip)
...
def crit_points(random_function, quantiles, sample_size, n_samples):
    ...
    """
    data = [[random_function() for _ in range(sample_size)] for __ in
            range(n_samples)]
